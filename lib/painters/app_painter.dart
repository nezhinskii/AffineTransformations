...
class AppPainter extends CustomPainter{
  ...
  static final _colors = List.generate(50, (_) => Color((Random().nextDouble() * 0xFFFFFF).toInt()).withOpacity(1.0));
  ...
  void paint(Canvas canvas, Size size) {
    _zBuffer = List.generate(size.height.toInt(), (_) => List.filled(size.width.toInt(), double.negativeInfinity));
    _pixels = List.generate(size.height.toInt(), (_) => List.filled(size.width.toInt(), null));
    ...
    for (int i = 0; i <  projectedPolyhedron.polygons.length; ++i) {
      ...
      for (var j = 1; j < projectedPolyhedron.polygons[i].points.length; ++j){
        canvas.drawLine(
          MainBloc.point3DToOffset(projectedPolyhedron.polygons[i].points[j - 1], size),
          MainBloc.point3DToOffset(projectedPolyhedron.polygons[i].points[j], size),
          _polyhedronPaint
        );
      }
      canvas.drawLine(
        MainBloc.point3DToOffset(projectedPolyhedron.polygons[i].points.first, size),
        MainBloc.point3DToOffset(projectedPolyhedron.polygons[i].points.last, size),
        _polyhedronPaint
      );
    }
    ...
    //     if (_pixels[i][j] != null){
    //       canvas.drawPoints(PointMode.points, [Offset(j.toDouble(), i.toDouble())], _paint..color = _pixels[i][j]!);
    //     }
    //   }
    // }
  }
  ...
  void drawTriangle({
    ...
    for (int i = 0; i < totalHeight; i++) {
      ...
      int segmentHeight = (secondHalf ? p1.dy - p2.dy : p0.dy - p1.dy).ceil() + 1;
      double alpha = i.toDouble()/totalHeight.toDouble();
      double beta  = (i - (secondHalf ? p0.dy-p1.dy : 0)).toDouble() / segmentHeight;
      ...
      for (int j = left.dx.floor(); j <= right.dx.ceil(); j++) {
        double ratio = (left.dx - right.dx).abs() < 1 ? 1 : (j - left.dx.toInt()).toDouble() / (right.dx - left.dx);
        Offset p = left + (right - left) * ratio;
        Offset point = Offset(p.dx.floorToDouble(), p.dy.floorToDouble());
        double z = leftZ + (rightZ - leftZ) * ratio;
        if (point.dy.toInt() > 0 && point.dy.toInt() < _zBuffer.length &&
            point.dx.toInt() > 0 && point.dx.toInt() < _zBuffer[0].length
